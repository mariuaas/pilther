...
export fn sierra2_dither(buf: [*]u8, width: c_int, height: c_int) c_int {
    ...
    for (0..h) |y| {
        for (0..w) |x| {
            ...
            if (x + 1 < w) diffuse(err_buf, idx + 1, diff, 4, 16);
            if (x + 2 < w) diffuse(err_buf, idx + 2, diff, 3, 16);
            if (y + 1 < h) {
                const row = idx + w;
                if (x >= 2) diffuse(err_buf, row - 2, diff, 1, 16);
                if (x >= 1) diffuse(err_buf, row - 1, diff, 2, 16);
                diffuse(err_buf, row, diff, 3, 16);
                if (x + 1 < w) diffuse(err_buf, row + 1, diff, 2, 16);
                if (x + 2 < w) diffuse(err_buf, row + 2, diff, 1, 16);
            }
        }
    }
