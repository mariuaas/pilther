...
pub fn stucki_dither(buf: [*]u8, width: c_int, height: c_int) c_int {
    ...
    for (0..h) |y| {
        ...
        for (0..w) |x| {
            ...
            if (x + 1 < w) diffuse(current, x + 1, diff, 8, 42);
            if (x + 2 < w) diffuse(current, x + 2, diff, 4, 42);
            if (y + 1 < h) {
                const next = err_buf.row(y + 1);
                if (x >= 2) diffuse(next, x - 2, diff, 2, 42);
                if (x >= 1) diffuse(next, x - 1, diff, 4, 42);
                diffuse(next, x, diff, 8, 42);
                if (x + 1 < w) diffuse(next, x + 1, diff, 4, 42);
                if (x + 2 < w) diffuse(next, x + 2, diff, 2, 42);
            }
            if (y + 2 < h) {
                const next2 = err_buf.row(y + 2);
                if (x >= 2) diffuse(next2, x - 2, diff, 1, 42);
                if (x >= 1) diffuse(next2, x - 1, diff, 2, 42);
                diffuse(next2, x, diff, 4, 42);
                if (x + 1 < w) diffuse(next2, x + 1, diff, 2, 42);
                if (x + 2 < w) diffuse(next2, x + 2, diff, 1, 42);
            }
        }
    }
